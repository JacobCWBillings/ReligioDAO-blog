...
export const ProposalListPage: React.FC = () => {
    ...
    const proposalStats = useMemo(() => {
        const activeCount = filteredProposals.filter((p) =>
            isActiveVoting(p),
        ).length;
        const executedCount = filteredProposals.filter(
            (p) => p.executed,
        ).length;
    ...
    return (
        <div className="proposal-list-page">
            <div className="proposals-header">
                ...
                <p>
                    Review and vote on blog submissions from the ReligioDAO
                    community
                </p>
                    ...
                    {filteredProposals.length > 0 ? (
                            ...
                            <div className="filter-summary">
                                <p>
                                    ...
                                    {totalCount > proposals.length
                                        ? ` (${proposals.length} of ${totalCount} loaded)`
                                        : ""}
                                </p>
                            ...
                            {shouldShowLoadMore && (
                                <div className="load-more-section">
                                    ...
                                    >
                                        {loadingMore
                                            ? "Loading more..."
                                            : `Load older proposals (${totalCount - proposals.length} remaining)`}
                                    </button>
                                </div>
                            )}
                        ...
                        <div className="no-proposals">
                            ...
                            {statusFilter !== "all" || searchTerm ? (
                                ...
                                <p>
                                    There are no proposals in the system yet. Be
                                    the first to submit one!
                                </p>
                            )}
                        </div>
                    )}
